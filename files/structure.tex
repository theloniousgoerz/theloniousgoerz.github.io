...
\setlength{\parindent}{1pt} % Stop paragraph indentation
...
\geometry{
	...
	headheight=2.2cm, % Header height
...
% \usepackage[semibold]{didone} % Use the EB Garamond font with a reduced bold weight
...
\sectionfont{\ExtraLarge {10pt}{10pt}\selectfont} % Set font options for sections
...
\setlength{\marginparsep}{-30pt} % Move the margin content closer to the text
\reversemarginpar % Margin text to be output into the left margin instead of the default right margin

% Create section heading with line for readability. 
\renewcommand{\sectionheading}[1]{%
  \bigskip \noindent
  \makebox[0pt][l]{\rule[-.5\baselineskip]{\linewidth}{.1mm}}%
  \textbf{#1}\par\nobreak 
  \vspace{0.01\textheight}
  \everypar{\hangindent=2em} % <--- This indents every subsequent line by 2em
}
...
\definecolor{header}{HTML}{8EACCD}
% Set link colours
\hypersetup{
	linkcolor=black,
	citecolor=red,
